...
static void JsonDecode(json_t * parent, ServiceAttributes& r, bool custom);
static void JsonEncode(const ServiceAttributes &r, json_t *parent);
 ...
 * ServiceAttributes :=
 * {
 *   "shortName"             : [String] SCVT short channel name,
 *   "providerImageURL"      : [String] service provider's Logo,
 *   "vcn"                   : [String] virtual channel number,
 * }
 *
 */
static void JsonEncode(const ServiceAttributes &r, json_t *parent)
{
	json_t *JT_serviceAttributes = parent;
	//Add Parent
    {
        if (!r.shortName.empty())
            json_object_set_new(JT_serviceAttributes, "shortName", json_string(r.shortName.c_str()));
        if (!r.providerImageURL.empty())
            json_object_set_new(JT_serviceAttributes, "providerImageURL", json_string(r.providerImageURL.c_str()));
        if (!r.vcn.empty())
            json_object_set_new(JT_serviceAttributes, "vcn", json_string(r.vcn.c_str()));
    }
}
...
static void JsonDecode(json_t * parent, ServiceAttributes& r, bool custom)
{
	//Decode parent
	{
		...
		if (custom) {
			json_t *JT_shortName = json_object_get(JT_activity, "CHANNEL_CALL_SIGN");
			const char * shortName = JT_shortName ? json_string_value(JT_shortName) : "";
			std::cout << "[DEC][CustomeAttributes] Short Name = "<< shortName << std::endl;

			json_t *JT_providerImageURL = json_object_get(JT_activity, "PROVIDER_IMAGE_URL");
			const char * providerImageURL = JT_providerImageURL ? json_string_value(JT_providerImageURL) : "";
			std::cout << "[DEC][CustomeAttributes] Provider Image URL = "<< providerImageURL << std::endl;

			json_t *JT_VCN = json_object_get(JT_activity, "CHANNEL_NUMBER");
			const char * vcn = JT_VCN ? json_string_value(JT_VCN) : "";
			std::cout << "[DEC][CustomeAttributes] Channel Number = "<< vcn << std::endl;
            
            r = ServiceAttributes(shortName, providerImageURL, vcn);
		}
        else {
			json_t *JT_shortName = json_object_get(JT_activity, "shortName");
			const char * shortName = JT_shortName ? json_string_value(JT_shortName) : "";
			std::cout << "[DEC][serviceAttributes] Short Name = "<< shortName << std::endl;

			json_t *JT_providerImageURL = json_object_get(JT_activity, "providerImageURL");
			const char * providerImageURL = JT_providerImageURL ? json_string_value(JT_providerImageURL) : "";
			std::cout << "[DEC][serviceAttributes] Provider Image URL = "<< providerImageURL << std::endl;

			json_t *JT_VCN = json_object_get(JT_activity, "vcn");
			const char * vcn = JT_VCN ? json_string_value(JT_VCN) : "";
			std::cout << "[DEC][serviceAttributes] Channel Number = "<< vcn << std::endl;
...
static void JsonDecode(json_t * parent, TunerReservation & reservation)
{
	//Decode parent
	{
		json_t *JT_reservation = parent;
		{
            ...
            if (TRM_USE_CUSTOM_ATTRIBUTES && JT_customAttributes && serviceLocator) {
                ServiceAttributes serviceAttributes;
                JsonDecode(JT_customAttributes, serviceAttributes, true/*custom*/);
                ServiceAttributes::set(serviceLocator, serviceAttributes);
            }
...
static void JsonEncode(const DetailedTunerState &r, json_t *parent)
{
	...
	//Add child
	{
		...
		if (!r.getServiceLocator().empty()) {
			...
			json_object_set_new(JT_detailedTunerState, "serviceAttributes", json_object());
            {
                JsonEncode(r.getServiceAttributes(), json_object_get(JT_detailedTunerState, "serviceAttributes"));
			}
		}
...
static void JsonDecode(json_t * parent, DetailedTunerState & detailedState)
{
	//Decode parent
	{
		json_t *JT_reservation = parent;
		{
			...
			json_t *JT_serviceAttributes  	= json_object_get(JT_reservation, "serviceAttributes");
            ...
            if (JT_serviceAttributes) {
                ServiceAttributes serviceAttributes;
				JsonDecode(JT_serviceAttributes, serviceAttributes , false /*custom*/);
                detailedState.setServiceAttributes(serviceAttributes);
            }
		}
	}
}
