...
bool renewRecord(const ReserveTuner &request, ReserveTunerResponse &response, int32_t clientId)
{
...
void reserveRecord(const ReserveTuner &request, ReserveTunerResponse &response, int32_t clientId)
{
	...
	if(!request.getTunerReservation().getReservationToken().empty()) {
		Log() << "Renew existing token with resurrect = " << (isResurrect ?  "true" : "false") << std::endl;
		response.getStatus() += "Renew existing token \r\n";
		if (renewRecord(request, response, clientId)) {
		    return;
        }
        else {
            Log() << "Renew failed, now look for new tuner\r\n";
        }
	}
	...
	if (!tunerToAddTo.empty())
	{
        ...
        std::string tokenToResurrect = request.getTunerReservation().getReservationToken();
        if (isResurrect && (!tokenToResurrect.empty())) {
            Log() << "Resurrect token " << tokenToResurrect << std::endl;
            recordReservation.setReservationToken(tokenToResurrect);
        }
        else {
...
void Execute(Executor<GetAllTunerStates> &exec)
{
	...
	try {
		...
		for (it=tunerIds.begin(); it != tunerIds.end(); it++) {
            ...
            if (ServiceAttributes::exist((Manager::getInstance().getTuner(*it).getServiceLocator()))) {
			response.addTunerState(*it,
					               ServiceAttributes::get((Manager::getInstance().getTuner(*it).getServiceLocator())));
            }
