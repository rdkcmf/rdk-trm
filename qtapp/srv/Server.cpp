...
Server::Server(const QHostAddress &address, quint16 port, bool start, bool sniff)
: started(false), serverAddress(address), serverPort(port), sniff(sniff)
{
	//QThreadPool::globalInstance()->setMaxThreadCount(10);
	tcpServer = new QTcpServer();
    
    /* defer listen() to ::start() */
    if (!start) return;
