...
NotifyTunerStatesUpdate Manager::getTunerStatesUpdate(void)
{
    ...
    for (tidIT = tunerIds.begin(); tidIT != tunerIds.end(); tidIT++) {
        ...
        if (ServiceAttributes::exist(tuner.getServiceLocator())) {
            detailedTunerState.setServiceAttributes(ServiceAttributes::get(tuner.getServiceLocator()));
        }
...
{
    Log() << "LocalTuner is set to " << tunerId << std::endl;
